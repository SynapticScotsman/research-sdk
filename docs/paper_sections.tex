...
replanning by 15 to 35 percent with no measurable cost in collisions, for
...
percent of visibility answers survive a replanning interval, yet only 0.4 per
...
search accounts for 2.0 percent of a visibility-graph call and 7.2 percent of
...
roadmaps replace 116 to 193 percent of their vertex set between consecutive
...
tessellation roadmaps replace 116 to 193 percent of their vertex set between
consecutive camera frames, so no previous search tree survives to repair. A
frozen-obstacle control returns 4 to 10 percent churn and an 18 percent
speedup, which establishes that the domain rather than the implementation
denies the saving. Since graph search is only 2.0 and 7.2 percent of a
...
run) for 39 percent of the collisions and a fifth of the planning time, and it
...
The control interval is $\Delta t=0.05$~s; it is not the prediction horizon.
The trigger sweep evaluates $H\in\{0.25,0.50,1.00\}$~s. At each decision,
obstacle velocity is estimated by finite differences over one control interval
and its position is projected to $H/4$, $H/2$, $3H/4$, and $H$.
The smoothed variant estimates velocity over five intervals ($0.25$~s) and
uses $H=0.25$~s. The saved speed sweeps compare the $0.25$~s predictive
variants; they do not evaluate a $0.05$~s prediction horizon.
...
trigger, with 95 percent bootstrap confidence intervals over the paired
differences.
...
\begin{tabular}{lrrr}
\hline
Obstacle speed (m/s) & Geometric & Predictive, $H=0.25$~s & Near-field, $K=2$ \\
\hline
...
roadmap, $n=100$ paired samples per cell, 23~mm measurement noise. The speed
sweep holds the prediction horizon at $H=0.25$~s, for both the plain and the
smoothed variant; the comparison across $H\in\{0.25,0.50,1.00\}$~s is the
trigger sweep. A $0.05$~s horizon was not evaluated.}
...
commanded direction reverses on 57.1 percent of replans. The robot is not
...
their whole length. Within these conditions, which hold the obstacle speed at
1.0~m/s, reversals per run orders arrival where neither the reversal rate nor
the replan count does: every trigger at or below 5.0 arrives in all 200 runs
and every trigger at or above 14.7 fails. That ordering does not hold once the
obstacle speed varies; see the text.}
\label{tab:stability}
\end{table}
...
of 200, while a 500~ms timer replans 30.2 times and fails in 32.

% provenance: results/stability-speed/, scripts/report_stability.py
Reversals per run separates arrival cleanly within these conditions, which all
hold the obstacle speed at 1.0~m/s: every condition arriving in all 200 runs
sits at or below 5.9, and every condition that fails sits at or above 14.7.
That separation does not survive a change of speed. Repeating the measurement
at 0.5 to 3.0~m/s in both scenarios, 396 conditions in total, the two groups
overlap: the probabilistic roadmap under a 50~ms timer reverses 12.0 times per
run and arrives in all 200, while the Voronoi roadmap under a 200~ms timer
fails at 11.1. We therefore report reversals per run as a description of what
the failing runs do, not as a predictor of failure.

What does separate over the full set is the combination of roadmap and trigger
family. All 48 conditions pairing the Voronoi roadmap with a timer fail at
every speed in both scenarios, and all 348 remaining conditions arrive in all
200 runs. Reversals are high in exactly the cases that fail, which is why the
count appeared to predict them, but the count alone does not.
...
The visibility graph is the stable backend, reversing on 0.0 to 1.4 percent of
...
league match, comparing two planning queries 500~ms apart, 98.2 percent of
visibility answers were unchanged, but only 0.4 percent could be established as
...
retaining 70.9 percent of its edges when all obstacles move 500~mm, because
obstacle motion perturbs the diagram locally.

Second, graph search is not where the cost lies. Measured separately on an
eight-obstacle scene, roadmap construction accounts for 98.0 percent of a
visibility-graph call (29.94~ms of 30.54~ms) and 92.8 percent of a Voronoi call
...
set added or removed between consecutive frames; above 100 percent the previous
...
mechanism is in the churn column. A visibility graph replaces 192.5 percent of
its vertex set per frame and a Voronoi roadmap 116.0 percent, so the repair
must diff a graph that has been almost entirely replaced. Only the probabilistic
roadmap retains its vertices (14.7 percent churn), because its milestones are
...
moves gives 4 to 10 percent churn, one vertex expanded per repair, and D* Lite
18 percent faster on the visibility graph, which confirms that the
...
ratio, 116 to 54 percent, which is what the argument predicts and would be
